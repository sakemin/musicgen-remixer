...
def save_numpy_as_audio(audio_data, sample_rate, output_filename):
    ...
    audio_segment = AudioSegment(audio_data.tobytes(), frame_rate=sample_rate, sample_width=audio_data.dtype.itemsize, channels=channels)
...
def mix_audio_volumes(audio1, audio2, weight1=0.5, weight2=0.5):
    ...
    mixed_audio_scaled = np.clip(mixed_audio * max_int16, -max_int16, max_int16).astype(np.int16)
...
def main(prompt, audio_path, model_version = "chord", beat_sync_threshold = None, upscale=False, mix_weight=0.65, output_path = "output"):
    """
    ...
    vocal_path = str(Path(output_path) / "WIPs" / Path(audio_path).name.rsplit(".", 1)[0]) + "_vocals.mp3"
    ...
    instrumental_path = str(Path(output_path) / "WIPs" / Path(audio_path).name.rsplit(".", 1)[0]) + "_inst.mp3"
    ...
    generated_instrumental_path = str(Path(output_path) / "WIPs" / Path(audio_path).name.rsplit(".", 1)[0]) + f"_{prompt}" + "_generated_inst.mp3"
    save_numpy_as_audio(generated_instrumental_track, generated_instrumental_sr, generated_instrumental_path)
    ...
    if not upscale:
        ...
        resampled_instrumental_track = resample_audio(generated_instrumental_track, generated_instrumental_sr, vocal_sr)
    ...
    resampled_instrumental_path = str(Path(output_path) / "WIPs" / Path(audio_path).name.rsplit(".", 1)[0]) + f"_{prompt}" + "_resampled_inst.mp3"
    ...
    for generated_downbeat in generated_downbeats:
        ...
        if len(aligned_input_downbeats) != 0 and int(input_beat * vocal_sr) == aligned_input_downbeats[-1]:
            print('Dropped')
            continue
    ...
    if downbeat_offset > 0:
        resampled_instrumental_track = np.concatenate([np.zeros([1,channel,int(downbeat_offset)]),resampled_instrumental_track],dim=-1)
        for i in range(len(aligned_generated_downbeats)):
            aligned_generated_downbeats[i]=aligned_generated_downbeats[i]+downbeat_offset
    ...
    for i in range(len(aligned_generated_downbeats)-1):
        s_ap += str(aligned_generated_downbeats[i]) + ":" + str(aligned_input_downbeats[i]) + ", "
    ...
    logger.info("Applying dynamic time-stretching on the generated instrumental track...")
    ...
    time_stretched_instrumental_track, time_stretched_instrumental_sr = download_audio_and_load_as_numpy(time_stretched_instrumental_track_url)

    ### Save the time-stretched instrumental track in mp3 format
    time_stretched_instrumental_path = str(Path(output_path) / "WIPs" / Path(audio_path).name.rsplit(".", 1)[0]) + f"_{prompt}" + "_time_stretched_inst.mp3"
    save_numpy_as_audio(time_stretched_instrumental_track, time_stretched_instrumental_sr, time_stretched_instrumental_path)
    logger.info(f"Saved time-stretched instrumental track to {time_stretched_instrumental_path}")
    ...
    if pad > 0:
        padded_instrumental_track = np.pad(time_stretched_instrumental_track, ((0, pad), (0, 0)), 'constant')
    else:
    ...
    mixed_track = mix_audio_volumes(padded_instrumental_track, vocal_track, weight1=mix_weight, weight2=1-mix_weight)

    ## Save the remixed track
    remixed_path = str(Path(output_path) / Path(audio_path).name.rsplit(".", 1)[0]) + f"_{prompt}" + "_remixed.mp3"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", dest="prompt", help="The prompt to use for generating the remix.")
    parser.add_argument("--audio_path", dest="audio_path", help="The path to the audio file to remix.")
    parser.add_argument("--model_version", dest="model_version", default="chord", help="The version of the model to use for generating the remix. [`chord`, `chord-large`, `stereo-chord`, `stereo-chord-large`]")
    parser.add_argument("--beat_sync_threshold", dest="beat_sync_threshold", default=None, help="The threshold for beat synchronization. If None, beat synchronization is automatically set to `1.1/(bpm/60)`.")
    parser.add_argument("--upscale", dest="upscale", action="store_true", help="Whether to upscale the audio to 48 kHz.")
    parser.add_argument("--mix_weight", dest="mix_weight", default=0.7, help="The weight for the generated instrumental track when mixing with the vocal.(0~1)")
    parser.add_argument("--output_path", dest="output_path", default="output", help="The path to save the output audio file.")
    args = parser.parse_args()
    main(args.prompt, args.audio_path, args.model_version, args.beat_sync_threshold, args.upscale, args.mix_weight, args.output_path)
